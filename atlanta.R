...
system("echo '' > atlanta.csv")
for(n in 1930:2015){
  eval(parse(text=paste(
    "system(\"wget -O 'file' 'http://www.wunderground.com/history/airport/KATL/",
    n,
    "/01/01/CustomHistory.html?dayend=31&monthend=12&yearend=",
    n,
    "&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&format=1'; cat file >> atlanta.csv\")"
    ,sep="")))
}
weather <- read.csv("atlanta.csv", header=TRUE, stringsAsFactors=FALSE)
