...
system("echo '' > minneapolis.csv")
for(n in 1945:2015){
  eval(parse(text=paste(
    "system(\"wget -O 'file' 'http://www.wunderground.com/history/airport/KMSP/",
    n,
    "/01/01/CustomHistory.html?dayend=31&monthend=12&yearend=",
    n,
    "&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&format=1'; cat file >> minneapolis.csv\")"
    ,sep="")))
}
weather <- read.csv("minneapolis.csv", header=TRUE, stringsAsFactors=FALSE)
names(weather)[1]<-"Date"
weather$city <- "Minneapolis (MN)"
...
write.csv(weather,"minneapolis.csv")
