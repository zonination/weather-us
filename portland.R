...
system("echo '' > portland.csv")
for(n in 1936:2015){
  eval(parse(text=paste(
    "system(\"wget -O 'file' 'http://www.wunderground.com/history/airport/KPDX/",
    n,
    "/01/01/CustomHistory.html?dayend=31&monthend=12&yearend=",
    n,
    "&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&format=1'; cat file >> portland.csv\")"
    ,sep="")))
}
weather <- read.csv("portland.csv", header=TRUE, stringsAsFactors=FALSE)
names(weather)[1]<-"Date"
weather$city <- "Portland (OR)"
...
write.csv(weather,"portland.csv")
