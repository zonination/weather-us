...
system("echo '' > losangeles.csv")
for(n in 1944:2015){
  eval(parse(text=paste(
    "system(\"wget -O 'file' 'http://www.wunderground.com/history/airport/KLAX/",
    n,
    "/01/01/CustomHistory.html?dayend=31&monthend=12&yearend=",
    n,
    "&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&format=1'; cat file >> losangeles.csv\")"
    ,sep="")))
}
weather <- read.csv("losangeles.csv", header=TRUE, stringsAsFactors=FALSE)
names(weather)[1]<-"Date"
weather$city <- "Los Angeles (CA)"
...
write.csv(weather,"losangeles.csv")

# # There's some kind of fuckery going on with LA. They decided that 
