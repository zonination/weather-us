...
system("echo '' > baltimore.csv")
for(n in 1945:2015){
  eval(parse(text=paste(
    "system(\"wget -O 'file' 'http://www.wunderground.com/history/airport/KBWI/",
    n,
    "/01/01/CustomHistory.html?dayend=31&monthend=12&yearend=",
    n,
    "&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&format=1'; cat file >> baltimore.csv\")"
    ,sep="")))
}
weather <- read.csv("baltimore.csv", header=TRUE, stringsAsFactors=FALSE)
names(weather)[1]<-"Date"
weather$city <- "Baltimore (MD)"
...
write.csv(weather,"baltimore.csv")
