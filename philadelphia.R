...
system("echo '' > philadelphia.csv")
for(n in 1941:2015){
  eval(parse(text=paste(
    "system(\"wget -O 'file' 'http://www.wunderground.com/history/airport/KPHL/",
    n,
    "/01/01/CustomHistory.html?dayend=31&monthend=12&yearend=",
    n,
    "&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&format=1'; cat file >> philadelphia.csv\")"
    ,sep="")))
}
weather <- read.csv("philadelphia.csv", header=TRUE, stringsAsFactors=FALSE)
names(weather)[1]<-"Date"
weather$city <- "Philadelphia (PA)"
...
write.csv(weather,"philadelphia.csv")
