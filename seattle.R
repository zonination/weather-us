...
system("echo '' > seattle.csv")
for(n in 1931:2015){
  eval(parse(text=paste(
    "system(\"wget -O 'file' 'http://www.wunderground.com/history/airport/KSEA/",
    n,
    "/01/01/CustomHistory.html?dayend=31&monthend=12&yearend=",
    n,
    "&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&format=1'; cat file >> seattle.csv\")"
    ,sep="")))
}
weather <- read.csv("seattle.csv", header=TRUE, stringsAsFactors=FALSE)
names(weather)[1]<-"Date"
weather$city <- "Seattle (WA)"
